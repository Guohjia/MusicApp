$(function(){
    ...
    $('.sitenav').on('tabChange',function(e,index){
        ...
        if(index===2){
            if($contentlis.eq(index).attr('data-downloaded')){console.log('请求过了');return}
            $.get('./tab3.json').then(function(result){
                let {content}=result
                // console.log(content)
                $contentlis.eq(index).html(content)
                $contentlis.eq(index).attr('data-downloaded','yes')
                // $('.contentLoading').remove()
            })
        }
